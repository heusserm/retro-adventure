...
// spellcheck: add fons et origo Beranek IFA's O'Dwyer RNG YAML
// spellcheck: add roguelike roguelikes dnd pedit5 Yob's
// spellcheck: add museumization ARPANET disambiguating
...
There is record of two earlier dungeon-crawling games written in
1974-75 on the PLATO system at University of Illinois <<DND>>,
<<PEDIT5>>.  These were in some ways similar to later roguelike games
but not to Adventure.  The designers of later roguelikes frequently
cite Adventure as an influence, but not dnd or pedit5; like PLATO itself, dnd
seems not to have become known outside of its own user community until
...
- [[[DND]]] https://en.wikipedia.org/wiki/Dnd_(video_game)[dnd (video
  game)]
